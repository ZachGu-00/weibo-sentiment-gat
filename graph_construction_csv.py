...
class BatchAPIManager:
    ...
    def wait_for_batch_completion(self, batch_id: str, check_interval: int = 30) -> dict:
        ...
        while True:
            ...
            in_progress = getattr(request_counts, 'in_progress', 0)
            ...
            self.logger.info(f"进度: {completed_requests}/{total_requests} ({progress_percent:.1f}%)")
            self.logger.info(f"处理中: {in_progress}")
            self.logger.info(f"失败: {failed_requests}")
            self.logger.info(f"已用时: {elapsed_time/60:.1f}分钟")
            
            if progress_percent > 0 and completed_requests > 0:
                # 估算剩余时间
                ...
                self.logger.info(f"预计剩余时间: {estimated_remaining_time/60:.1f}分钟")
            
            # 检查是否完成
            if status in ["completed", "failed", "expired", "cancelled"]:
                self.logger.info(f"\n=== 任务结束 ===")
                self.logger.info(f"最终状态: {status}")
                
                if status == "completed":
                    self.logger.info(f"✅ 任务成功完成！")
                    self.logger.info(f"输出文件ID: {batch.output_file_id}")
                    if batch.error_file_id:
                        self.logger.info(f"错误文件ID: {batch.error_file_id}")
                elif status == "failed":
                    self.logger.error(f"❌ 任务失败")
            ...
            # 等待下次检查
            self.logger.info(f"等待{check_interval}秒后再次检查...")
...
class CommentGraphAnalyzer:
    ...
    def load_comment_data(self, filepath):
        ...
        try:
            ...
            # 解析视频发布日期
            if '视频发布日期' in df.columns:
                df['视频发布日期'] = pd.to_datetime(df['视频发布日期'])
            return df
        except Exception as e:
            self.logger.error(f"加载评论数据失败: {filepath}, 错误: {e}")
            return None
    
    def compute_node_features(self, comment_row, video_publish_date):
        """计算单个节点的特征"""
        features = {}
        ...
        features['video_age_when_commented'] = (comment_time - video_publish_date).days
    ...
    def analyze_interaction_pair(self, parent_row, reply_row, stance=None, styles=None) -> Dict:
        """分析单个互动对的特征（立场信息可选）"""
        parent_comment = str(parent_row['评论内容'])
        ...
        labels = {
            'parent_id': parent_id,
            'reply_id': reply_id,
            'parent_likes': float(parent_row['点赞数']) if pd.notna(parent_row['点赞数']) else 0.0,
            'reply_likes': float(reply_row['点赞数']) if pd.notna(reply_row['点赞数']) else 0.0,
            'parent_replies': float(parent_row['回复数']) if pd.notna(parent_row['回复数']) else 0.0,
            'reply_replies': float(reply_row['回复数']) if pd.notna(reply_row['回复数']) else 0.0
        }
        
        return features, labels
    
    def build_comment_tree_with_batch(self, comment_data, root_comment_id, file_source, video_publish_date, stance_results=None):
        """构建子图（支持批量立场结果）"""
        G = nx.DiGraph()
        ...
        for idx, row in comment_data.iterrows():
            comment_id = row['评论ID']
            features = self.compute_node_features(row, video_publish_date)
        ...
        edge_stats = {
            ...
            'avg_parent_likes': np.mean([labels['parent_likes'] for labels in edge_labels.values()]) if edge_labels else 0,
            'avg_reply_likes': np.mean([labels['reply_likes'] for labels in edge_labels.values()]) if edge_labels else 0,
            'avg_parent_replies': np.mean([labels['parent_replies'] for labels in edge_labels.values()]) if edge_labels else 0,
            'avg_reply_replies': np.mean([labels['reply_replies'] for labels in edge_labels.values()]) if edge_labels else 0
        }
        ...
        graph_data = {
            ...
            'video_publish_date': video_publish_date.strftime('%Y-%m-%d'),
    ...
    def process_batch(self) -> None:
        ...
        if self.config.use_batch_api:
            ...
            for csv_file in files_to_process:
                ...
                # 获取视频发布日期
                video_publish_date = comment_data['视频发布日期'].iloc[0] if '视频发布日期' in comment_data.columns else self.config.data_collection_date
    ...
    def process_single_file(self, filepath):
        ...
        try:
            file_source = f"{bv_id}_{root_comment_id}"
            graph_data = self.build_comment_tree_with_batch(
                comment_data, int(root_comment_id), file_source, video_publish_date, None
            )
